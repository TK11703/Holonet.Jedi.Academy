...
namespace Holonet.Jedi.Academy.BL.Dashboards
{
	public class KnowledgeDashboard
	{
		...
		public KnowledgeDashboard(SiteConfiguration config, string userTZOffset)
		{
			Config = config;
			UserTZOffset = userTZOffset;
		}

		public ChartResponse<decimal> GetSkillParticipation(List<KnowledgeXP> skillAcceptances, string chartTitle)
		{
			...
			var list = from skill in skillAcceptances
					   group skill by skill.Status into grp
		...
		public double GetSkillAvgCompletion(List<KnowledgeXP> skillAcceptances)
		{
			double avgRotations = 0;
			if (skillAcceptances.Count() > 0)
			{
				avgRotations = skillAcceptances.Average(x => (x.CompletedOn.Value.Date - x.StartedOn.Date).TotalDays);
			}
			return avgRotations;
		}
	}
