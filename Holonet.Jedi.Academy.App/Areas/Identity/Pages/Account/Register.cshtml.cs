...
namespace Holonet.Jedi.Academy.App.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            {
                var user = new JediAcademyAppUser { UserName = Input.Email, Email = Input.Email, FirstName = Input.FirstName, LastName = Input.LastName };
                var result = await _userManager.CreateAsync(user, Input.Password);
                if (result.Succeeded)
                {
                    _logger.LogInformation("User created a new account with password.");
                    var defaultrole = _roleManager.FindByNameAsync(Identity.Enums.Roles.Student.ToString()).Result;
                    if (defaultrole != null)
                    {
